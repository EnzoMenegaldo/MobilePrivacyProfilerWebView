...
app.post('/SMS', (req, res) => {
    db = startConnection (dbAdress)
    console.log('SMS stats requested. UserId : '+req.body.UserId)
    var userId = ''+req.body.UserId
    db.all(
...
app.post('/GPSGeolocation', (req, res) => {
    db = startConnection (dbAdress)
    console.log('GPS Geolocation requested. UserId : '+req.body.UserId)
    var userId = ''+req.body.UserId
    db.all(
        'SELECT _id, date, latitude, longitude, precision, altitude, verticalPrecision FROM geolocation WHERE userId= ?',
        [userId],
        function(error, rows) {
            res.send(rows)
        }
